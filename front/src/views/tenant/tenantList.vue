  ...
  <div class="app" style="height: 100vh; width: 100%; margin: 0 auto;">
    ...
    <div>
      <el-form :inline="true" :model="attendeeQuery" class="demo-form-inline" ref="attendeeQuery">
        ...
        <el-form-item label="租户名称" prop="username">
          <el-input v-model="attendeeQuery.username" placeholder="输入租户名称"></el-input>
        </el-form-item>

        <el-form-item label="联系人" prop="nickname">
          <el-input v-model="attendeeQuery.nickname" placeholder="请输入联系人"></el-input>
        </el-form-item>
    ...
    <el-table ref="multipleTable"
      ...
      <el-table-column align="center"
        ...
        <template v-slot="scope">
          <!-- 编辑按钮 -->
          <router-link :to="'/editTenant/' + scope.row.id">
          ...
          <el-button style="margin-left: 10px;"
                     size="small"
                     type="danger"
                     @click="deleteTenant(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 一键删除多个用户
    deleteMore() {
      this.$confirm('此操作将删除多个租户，是否继续？', '提示', {
        ...
        // 调用删除多个用户的接口
        attendee.deleteMoreTenant(ids).then((_) => {
          this.$notify({
            title: '删除成功',
            type: 'success',
          })
    ...
    // 查询用户列表
    findTenantList() {
      tenant.findTenantList(this.pageParam.index, this.pageParam.size, this.attendeeQuery).then((res) => {
        console.log(res);  // 查看后端返回的完整响应
        this.tenantList = res.data.userList;
    ...
    // 删除单个用户
    deleteTenant(id) {
      this.$confirm('此操作将删除改租户, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
      }).then(() => {
        // 调用删除接口
        tenant.deleteTenant(id).then((_) => {
          this.$message({
            type: 'success',
            message: '删除成功!',
          })
